-- models/customer_targeting.sql
-- Common Table Expression (CTE) to transform staging_store data
with
...
select
    ...
    to_varchar(
        floor(cast(regexp_replace(product_sku, '[vV]', '') as numeric))
    ) as product_sku,
    upper(trim(regexp_replace((category_name), '[.#,]', ''))) as category_name,
    amount as amount,
    upper(trim(status)) as status,
    to_varchar(
        lpad(cast(regexp_replace(card_number, ' ', '') as bigint), 19, '0')
    ) as card_number,
